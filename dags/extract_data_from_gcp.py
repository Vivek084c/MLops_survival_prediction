...
def load_to_sql(file_path):
    conn = BaseHook.get_connection('postgress_default')  
    engine = sqlalchemy.create_engine(f"postgresql+psycopg2://{conn.login}:{conn.password}@m-lops-survival-prediction_f73acb-postgres-1:{conn.port}/{conn.schema}")
